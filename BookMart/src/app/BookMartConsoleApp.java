...
public class BookMartConsoleApp {
	...
	String[] memberMenuList = { "로그아웃", "도서 목록", "도서 주문", "주문 목록", "장바구니 도서 담기", "장바구니 보기", "내 정보" };
	...
	BookService bs = new HWBookService(new ObjFileHashMapBookDAO());
	MemberService ms = new HWMemberService(new ObjFileHashMapMemberDAO());
	...
	private void displayWelcome() {
		System.out.println("*********************************");
		System.out.println("* Wlecome to Hyunwoo Book Mart  *");
		System.out.println("*********************************");
	}
	...
	private void menuExit() {
		System.out.println("Hyunwoo Book Mart 서비스를 종료합니다.");
	}

	private void menuBookList() {
		System.out.println("*** 도석 목록 ***");
		displayBookList();
	}
	...
	private void menuAddBook2Cart() {
		...
		// 장바구니에 있나?
		// 없으면, 장바구니에 담기
		if (cs.getCartItemInfo(bookNo) == null) {
			cs.addItem2Cart(new CartItemVO(bookNo, quantity));
			System.out.println("장바구니에 추가했습니다.");
		} else {
			System.out.println("이미 장바구니에 있는 도서 입니다.");
		}
	...
	private void menuCartOrder() {
		...
		int totalPrice = 0;
		for (CartItemVO item : cs.listCartItems()) {
			BookVO book = bs.detailBookInfo(item.getBookNo());
			int price = book.getPrice() * item.getQuantity();
			totalPrice += price;
			orderItemList.add(new OrderItemVO(item.getBookNo(), item.getQuantity(), price));
		}
	...
	private void menuCartBookDelete() {
		System.out.println("*** 장바구니 도서 삭제 ***");
		displayCartItemList();
		int bookNo = input.readInt(">> 도서 번호 : ");
		CartItemVO item = cs.getCartItemInfo(bookNo);

		if (item == null) {
			System.out.println("없는 도서입니다.");
		} else {
			cs.removeCartItem(bookNo);
			System.out.println("장바구니에서 해당 도서를 삭제하였습니다.");
		}
		displayCartItemList();
	...
	private void menuCartClear() {
		...
		String confirm = input.readString(">> 장바구니의 모든 도서를 삭제 하시겠습니까? ('" + CONFIRM + "'이면 삭제) : ");

		if (confirm.equals(CONFIRM)) {
			cs.clearCart();
			System.out.println("장바구니의 모든 도서를 삭제하였습니다.");
		} else {
